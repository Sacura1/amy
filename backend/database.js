...
const raffles = {
    ...
    create: async (title, description, imageUrl, countdownHours, createdBy, thresholdPoints = 5000, thresholdParticipants = 10) => {
        if (!pool) return null;
        const result = await pool.query(
            `INSERT INTO raffles (title, prize_description, image_url, countdown_hours, created_by, threshold_points, threshold_participants)
             VALUES ($1, $2, $3, $4, LOWER($5), $6, $7)
             RETURNING *`,
            [title, description, imageUrl, countdownHours, createdBy, thresholdPoints || 5000, thresholdParticipants || 10]
        );
